function dydt = sistema_aerodinamico(t, y, aeronave, tempo_controle, controle)
    ...
    if t >= tempo_controle(2) && t <= tempo_controle(3)
        delta_aileron = -controle.delta_aileron; % Aplicar a deflexão máxima
    elseif t >= tempo_controle(1) && t < tempo_controle(2)
        delta_aileron = controle.delta_aileron; % Aplicar deflexão parcial
    else
        delta_aileron = 0; % Fora do intervalo, a deflexão é zero
    end
...
aeronave.Cm_ref = 0.00004;   % Coeficiente de momento pitch (sem deflexão)
...
aeronave.CL_aileron = 0.000000;    aeronave.CL_rudder = -0.000000;    aeronave.CL_elevator = 0.005507;
aeronave.CY_aileron = -0.001034;    aeronave.CY_rudder = -0.003119;    aeronave.CY_elevator = 0.000000;
aeronave.Cl_aileron = -0.007066;    aeronave.Cl_rudder = -0.000201;    aeronave.Cl_elevator = 0.000000;
aeronave.Cm_aileron = 0.000000;     aeronave.Cm_rudder = 0.000000;     aeronave.Cm_elevator = -0.021509;
aeronave.Cn_aileron = -0.000284;    aeronave.Cn_rudder = 0.001274;     aeronave.Cn_elevator = -0.000000;

% Parâmetros de controle (exemplo simples)
t_span = [0 50];  % Intervalo de tempo para a simulação
tempo_controle = [20 25 30];  % Intervalos de controle
controle.delta_elevator = 0.1; % Deflexão do elevator (rad)
controle.delta_aileron = 0.1;  % Deflexão do aileron (rad)
controle.delta_rudder = 0.05;  % Deflexão do leme (rad)

% Inicialização do estado da aeronave
V_0 = 48.34; % m/s
alfa_ref = deg2rad(0.19); %°
betha_ref = 0; %°
h = 3000; % m
...
aeronave.alfa_ref = alfa_ref;
aeronave.elevator_ref = -0.41;
...
xlabel('Tempo (s)');
ylabel('p (rad/s)');
ylim([-1 1]); % Define a escala vertical fixa para p
...
xlabel('Tempo (s)');
ylabel('q (rad/s)');
ylim([-1 1]); % Define a escala vertical fixa para q
...
xlabel('Tempo (s)');
ylabel('r (rad/s)');
ylim([-1 1]); % Define a escala vertical fixa para r
...
xlabel('Tempo (s)');
ylabel('\phi (rad)');
ylim([-pi pi]); % Define a escala vertical fixa para \phi
...
xlabel('Tempo (s)');
ylabel('\psi (rad)');
ylim([-pi pi]); % Define a escala vertical fixa para \psi
