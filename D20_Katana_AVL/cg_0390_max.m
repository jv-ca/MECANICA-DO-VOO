function dydt = sistema_aerodinamico(t, y, aeronave, tempo_controle, controle)
    ...
    if t >= tempo_controle(2) && t <= tempo_controle(3)
        delta_rudder = 0; % Aplicar a deflexão máxima
    elseif t >= tempo_controle(1) && t < tempo_controle(2)
        delta_rudder = controle.delta_rudder; % Aplicar deflexão parcial
    else
        delta_rudder = 0; % Fora do intervalo, a deflexão é zero
    end
...
aeronave.rho = 0.909;       % Densidade do ar (kg/m³)
...
aeronave.CL_ref = 0.27936;   % Coeficiente de sustentação (sem deflexão)
aeronave.CD_ref = 0.00318;   % Coeficiente de arrasto (sem deflexão)
aeronave.CY_ref = 0.00000;   % Coeficiente de lado (sem deflexão)
aeronave.Cl_ref = 0.00000;   % Coeficiente de momento roll (sem deflexão)
aeronave.Cm_ref = 0.00001;   % Coeficiente de momento pitch (sem deflexão)
...
aeronave.CL_alfa = 4.168075;   aeronave.CL_beta = 0.000000;
aeronave.CY_alfa = -0.000000;  aeronave.CY_beta = -0.288412;
aeronave.Cl_alfa = -0.000000;  aeronave.Cl_beta = -0.125813;
aeronave.Cm_alfa = -1.163820;  aeronave.Cm_beta = -0.000000;
aeronave.Cn_alfa = 0.000000;   aeronave.Cn_beta = 0.081766;

% Derivadas em relação às taxas angulares p, q, r
aeronave.CL_p = -0.000000;    aeronave.CL_q = 7.382406;    aeronave.CL_r = -0.000000;
aeronave.CY_p = -0.137639;    aeronave.CY_q = 0.000000;     aeronave.CY_r = 0.237902;
aeronave.Cl_p = -0.435562;    aeronave.Cl_q = -0.000000;    aeronave.Cl_r = 0.090764;
aeronave.Cm_p = 0.000000;     aeronave.Cm_q = -15.012995;   aeronave.Cm_r = 0.000000;
aeronave.Cn_p = -0.024507;    aeronave.Cn_q = -0.000000;    aeronave.Cn_r = -0.076985;

% Derivadas de estabilidade para aileron, elevator e rudder
% Multiplicar todas 180/pi
conversor_deg_rad = 180/pi;
aeronave.CL_aileron = 0.000000 * conversor_deg_rad;    aeronave.CL_rudder = -0.000000 * conversor_deg_rad;    aeronave.CL_elevator = 0.005502 * conversor_deg_rad;
aeronave.CY_aileron = -0.001045 * conversor_deg_rad;    aeronave.CY_rudder = -0.003110 * conversor_deg_rad;    aeronave.CY_elevator = 0.000000 * conversor_deg_rad;
aeronave.Cl_aileron = -0.007061 * conversor_deg_rad;    aeronave.Cl_rudder = -0.000198 * conversor_deg_rad;    aeronave.Cl_elevator = 0.000000 * conversor_deg_rad;
aeronave.Cm_aileron = 0.000000 * conversor_deg_rad;     aeronave.Cm_rudder = 0.000000 * conversor_deg_rad;     aeronave.Cm_elevator = -0.021039 * conversor_deg_rad;
aeronave.Cn_aileron = -0.000331 * conversor_deg_rad;    aeronave.Cn_rudder = 0.001243 * conversor_deg_rad;     aeronave.Cn_elevator = -0.000000 * conversor_deg_rad;
...
controle.delta_elevator = deg2rad(-2.35); % Deflexão do elevator (rad)
controle.delta_aileron = deg2rad(0);  % Deflexão do aileron (rad)
controle.delta_rudder = deg2rad(3);  % Deflexão do leme (rad)

% Inicialização do estado da aeronave
V_0 = 48.34; % m/s
alfa_ref = deg2rad(1.114); %°
betha_ref = 0; %°
h = 3000; % m
